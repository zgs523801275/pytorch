...
class _DenseBlock(nn.Module):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate, memory_efficient=False):
        ...
        for i in range(num_layers):
            layer = _DenseLayer(
                #denselayer的输入特征维度变化：每一级denselayer输入特征维度增加growth_rate
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        for i, num_layers in enumerate(block_config):
            ...
            #每一级denseblock输入特征维度变化：
            #num_features->((num_features + num_layers * growth_rate) // 2)
            #例如denseNet121：
            #第一级denseblock输入特征维度：64(初始值)
            #第二级denseblock输入特征维度：(64 + 6 * 32) // 2 = 128
            #第三级denseblock输入特征维度：(128 + 12 * 32) // 2 = 256
            #第四级denseblock输入特征维度：(256 + 24 * 32) // 2 = 512
